*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVSA05PRO
*   generation date: 2022.07.21 at 17:07:56
*   view maintenance generator version: #001407#
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZVSA05PRO           .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVSA05PRO'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVSA05PRO_EXTRACT
TOTAL = ZVSA05PRO_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVSA05PRO'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVSA05PRO-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVSA05PRO_EXTRACT
TOTAL = ZVSA05PRO_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVSA05PRO-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVSA05PRO'
...
TOTAL = ZVSA05PRO_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVSA05PRO-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVSA05PRO-UPD_FLAG NE SPACE.
